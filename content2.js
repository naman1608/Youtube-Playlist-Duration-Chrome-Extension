...
function check(a) {
    console.log(a);
    if (a < playlist_size) setTimeout(() => check(document.querySelectorAll(".style-scope ytd-thumbnail-overlay-time-status-renderer").length), 1000);
    else document.getElementById("publisher-container").insertAdjacentHTML("beforeEnd", "<span id='totaltime' class='publisher style-scope ytd-playlist-panel-renderer complex-string yt-simple-endpoint style-scope yt-formatted-string'> &nbsp; - " + add_total_time() + "</span>");
}
